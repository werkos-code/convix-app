...
import { fixedExpenseSchema } from "@/lib/validations/schemas";
import { fail, ok, requireCents, type ActionResult } from "./_result";

async function rematerializeOpenAndNext(
  supabase: Awaited<ReturnType<typeof requireUser>>["supabase"],
  userId: string,
) {
  const { period } = await ensureOpenPeriod(supabase, userId);
...
export async function createFixedExpenseRule(input: {
  name: string;
  amountEuros: string;
  category?: string;
  ...
  try {
    ...
    if (!parsed.success) {
      return fail(
        parsed.error.issues[0]?.message ?? "Ongeldige vaste last",
      );
    }

    if (parsed.data.recurrence === "yearly" && !parsed.data.monthOfYear) {
      return fail("Kies een maand voor jaarlijkse lasten");
    }
    ...
    return fail(
      e instanceof Error ? e.message : "Vaste last opslaan mislukt",
    );
  }
}

/** Soft-deactivate a rule and cancel open (unsettled) obligations from it. */
export async function deactivateFixedExpenseRule(input: {
  id: string;
}): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    await supabase
      ...
      .eq("user_id", user.id)
      .eq("source_type", "fixed_expense_rule")
    ...
    return fail(
      e instanceof Error ? e.message : "Vaste last verwijderen mislukt",
    );
  }
}
